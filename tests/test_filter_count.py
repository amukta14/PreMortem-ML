import premortemml.multilabel_classification.dataset
from premortemml import count, filter
from premortemml.count import (
    get_confident_thresholds,
    estimate_py_and_noise_matrices_from_probabilities,
)
from premortemml.internal.latent_algebra import compute_inv_noise_matrix
from premortemml.benchmarking.noise_generation import generate_noise_matrix_from_trace
from premortemml.benchmarking.noise_generation import generate_noisy_labels
from premortemml.internal.util import value_counts
...
def test_find_label_issues_multi_label(
    ...
    if multi_label:
        if use_dataset_function:
            noise_idx = premortemml.multilabel_classification.filter.find_label_issues(
                labels=dataset["labels"],
                pred_probs=dataset["pred_probs"],
                filter_by=filter_by,
    ...
    # Make sure premortemml does reasonably well finding the errors.
...
def test_issue_158():
    # ref: https://github.com/premortemml/premortemml/issues/158
    pred_probs = np.array(
        [
